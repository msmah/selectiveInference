...
fixedLassoInf <- function(x, y, beta, 
  ...
  if(family=="binomial")  {
    ...
    if (intercept==TRUE) {
	    message('glmnet was run with an intercept term, centering x and y ...\n')
	...
	message('Checking the KKT conditions ...\n')
    ...
    if (max(A %*% y - b) > tol.poly * sqrt(sum(y^2)))
      stop(paste("Polyhedral constraints not satisfied; you must recompute beta",
                 ...
                 "'thresh' parameter, for a more accurate convergence."))
    message('Polyhedral region checked.\n')
    # Estimate sigma
    if (is.null(sigma)) {
      if (n >= 2*p) {
        oo = intercept
    ...
    if (type=="full" & p > n) {
      ...
      GS = cbind(diag(length(S)),matrix(0,length(S),pp-length(S)))
	    ...
	    message('Reorder so that active set S is first DONE\n')
      ...
      if (!is_wide) {
	    ...
	    message('Approximate inverse covariance matrix for when (n < p) from lasso_Inference.R DONE')

      M <- (((htheta%*%t(Xordered))+ithetasigma%*%FS%*%hsigmaSinv%*%t(XS))/n)
	    message('M was calculated\n')
...
fixedLassoPoly =
  function(X, y, lambda, beta, active, inactive = FALSE) {
    ...
    message('fixedLassoPoly was DONE\n')
    return(list(A=A, b=b))
  }
